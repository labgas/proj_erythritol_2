...
/*----------------------------*/
/* check distribution Glucose */
/*----------------------------*/
proc univariate data=PT_hormone;
var glucose_mmol_L; 
histogram glucose_mmol_L/ normal (mu=est sigma=est) lognormal (sigma=est theta=est zeta=est);
run;
...
proc transreg data=PT_hormone maxiter=0 nozeroconstant;
   	model BoxCox(glucose_mmol_L/parameter=0) = identity(z);
run;
...
data PT_hormone;
set PT_hormone;
bc_glucose_mmol_L = ((glucose_mmol_L)**-2 -1)/-2;
run;
...
proc univariate data=PT_hormone;
var bc_glucose_mmol_L;
histogram bc_glucose_mmol_L / normal (mu=est sigma=est);
run;
...
model bc_glucose_mmol_L = condition | time_cond/ ddfm=kr2 solution influence residual;
...
class subject condition time_cond;
where condition NE 'sucrose';
model bc_glucose_mmol_L = condition | time_cond | dosage / ddfm=kr2 influence solution residual;
